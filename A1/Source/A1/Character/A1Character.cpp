...
void AA1Character::SkillHitCheck(float SkillRange)
{
	...
	bool bHitResult = GetWorld()->SweepMultiByChannel(OutHitResults, Origin, Origin,
	...
	if (Montage && Montage->GetFName() == TEXT("AM_Skill"))
	{
		...
		GetWorld()->GetTimerManager().SetTimer(
			TimerHandle, FTimerDelegate::CreateLambda(
				...
				{
					bUseSkill = true;
					GEngine->AddOnScreenDebugMessage(-1, 1.f, FColor::Red, TEXT("Use Skill"));
				}),
			SkillCoolTime,
			false);
	}
...
{
	GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Blue, TEXT("Skill"));
